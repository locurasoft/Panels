...
function newParser()
    ...
    function XmlParser:FromXmlString(value)
        value = string.gsub(value, "&#x([%x]+)%;",
            function(h)
                return string.char(base.tonumber(h, 16))
            end);
        value = string.gsub(value, "&#([0-9]+)%;",
            function(h)
                return string.char(base.tonumber(h, 10))
            end);
        value = string.gsub(value, "&quot;", "\"");
        value = string.gsub(value, "&apos;", "'");
        value = string.gsub(value, "&gt;", ">");
    ...
    function XmlParser:loadFile(xmlFilePath)
        local hFile, err = io.open(xmlFilePath, "r");
...
function newNode(name)
    ...
    function node:addChild(child)
        if self[child:name()] ~= nil then
            if base.type(self[child:name()].name) == "function" then
                local tempTable = {}
    ...
    function node:addProperty(name, value)
        ...
        if self[lName] ~= nil then
            if base.type(self[lName]) == "string" then
                local tempTable = {}
