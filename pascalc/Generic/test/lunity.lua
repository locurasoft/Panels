...
local flush = function(text)
	write(text)
	local printer = output or print
	printer(_output)
	_output = ""
end
...
function __runAllTests( testSuite, options )
	...
	if useHTML then
		flush( "<h2 style='background:#000; color:#fff; margin:1em 0 0 0; padding:0.1em 0.4em; font-size:120%'>"..testSuite._NAME.."</h2><pre style='margin:0; padding:0.2em 1em; background:#ffe; border:1px solid #eed; overflow:auto'>" )
	else
		flush( string.rep('=',78) )
		flush( testSuite._NAME )
		flush( string.rep('=',78) )
	end
	io.stdout:flush()
	...
	for _,testName in ipairs(theTestNames) do
		...
		if successFlag then
			...
			if useANSI then
				flush( "\27[31m\27[1mFAIL!\27[0m" )
				flush( "\27[31m"..errorMessage.."\27[0m" )
			elseif useHTML then
				flush("<b style='color:red'>FAIL!</b>")
				flush( "<span style='color:red'>"..errorMessage.."</span>" )
			else
				flush("FAIL!")
				flush( errorMessage )
			end
	...
	flush( string.format( "%d/%d tests passed (%0.1f%%)",
		theSuccessCount,
		#theTestNames,
	...
	flush( string.format( "%d total successful assertion%s",
