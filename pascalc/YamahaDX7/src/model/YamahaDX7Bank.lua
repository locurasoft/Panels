...
require("message/DX7SyxMsg")
require("Logger")
require("lutils")

local log = Logger("YamahaDX7Bank")

YamahaDX7Bank = {}
YamahaDX7Bank.__index = YamahaDX7Bank

setmetatable(YamahaDX7Bank, {
...
function YamahaDX7Bank:_init(bankData)
  ...
  if bankData == nil then
    ...
    for i = 0, NUM_PATCHES - 1 do
      table.insert(self.patches, YamahaDX7Patch())
    end
  else
    assert(bankData:getSize() == BANK_BUFFER_SIZE, string.format("Data does not contain a Yamaha DX7 bank"))
    self.data = MemoryBlock(BANK_BUFFER_SIZE, false)
    self.data:copyFrom(bankData, 0, BANK_BUFFER_SIZE)

    for i = 0, NUM_PATCHES - 1 do
      table.insert(self.patches, YamahaDX7Patch(getPatch(self.data, i)))
    end
  end
end

function YamahaDX7Bank:toStandaloneData()
  compressData(self.patches, self.data)
  return self.data
end

function YamahaDX7Bank:toSyxMessages()
  compressData(self.patches, self.data)
  return { DX7SyxMsg(self.data) }
end
