...
function onPanelSaveState(stateData)
  for key, modelName in ipairs(MODEL_NAMES) do
    local varName = modelName:gsub("^%l", string.lower)
...
function onPanelRestoreState(stateData)
  for key, modelName in ipairs(MODEL_NAMES) do
    local varName = modelName:gsub("^%l", string.lower)
    _G[varName] = stateData:getProperty(varName)
  end
end
...
function onPanelLoaded(type)
  ...
  drumMapController     = DrumMapController(drumMap)
...
function onPanelBeforeLoad()
  ...
  PATH_SEPARATOR = cutils.getPathSeparator()
  EOL = cutils.getEolChar()
  OPERATING_SYSTEM = cutils.getOsName()
